...
public class DefaultDesktop extends TabbedDesktop implements MenuListener, Serializable, EventListener<Event>, EventHandler, DesktopCleanup
{
            ...
            e.addEventListener(Events.ON_OPEN, new EventListener<Event>() {
    			...
    			public void onEvent(Event event) throws Exception {
    				...
    				if (comp != null) {
    					...
    					if (!oe.isOpen())
    						layout.getWest().setVisible(false);
    				}
    			}
    		});
	...
	protected Component doCreatePartRTL(Component parent)
    {
    	PageDefinition pagedef = Executions.getCurrent().getPageDefinition(ThemeManager.getThemeResource("zul/desktop/desktop.zul"));
    	Component page = Executions.createComponents(pagedef, parent, null);
    	layout = (Borderlayout) page.getFellow("layout");
        ...
        East e1 = layout.getEast();
        e1.addEventListener(Events.ON_OPEN, new EventListener<Event>() {
			@Override
			public void onEvent(Event event) throws Exception {
				OpenEvent oe = (OpenEvent) event;
				updateMenuCollapsedPreference(!oe.isOpen());				
        ...
        e1.addEventListener(Events.ON_SWIPE, new EventListener<SwipeEvent>() {
        ...
        e1.addEventListener(Events.ON_SIZE, new EventListener<Event>() {
        ...
        UserPreference pref = SessionManager.getSessionApplication().getUserPreference();
        boolean menuCollapsed= pref.isPropertyBool(UserPreference.P_MENU_COLLAPSED);
        e1.setOpen(!menuCollapsed);
        if (!e1.isOpen())
        	LayoutUtils.addSclass("slide", e1);
        
        mobile = ClientInfo.isMobile();
    	e1.setCollapsible(true);
    	LayoutUtils.addSlideSclass(e1);

        if (mobile) {
        	e1.setOpen(false);
        }

        West w1 = layout.getWest();
       
        w1.addEventListener(Events.ON_OPEN, new EventListener<Event>() {
			@Override
			public void onEvent(Event event) throws Exception {
				OpenEvent oe = (OpenEvent) event;
				updateHelpCollapsedPreference(!oe.isOpen());
        ...
        w1.addEventListener(Events.ON_SWIPE, new EventListener<SwipeEvent>() {
        ...
        w1.addEventListener(Events.ON_SIZE, new EventListener<Event>() {
        ...
        if( westWidth != null || eastWidth != null ){
        	...
        	if( westWidth != null && eastWidth != null ){
        		...
        		if( eWidth + wWidth <= browserWidth ){
        			ZKUpdateUtil.setWidth(e1, westWidth);
        			ZKUpdateUtil.setWidth(w1, eastWidth);
        		}
        ...
        boolean helpCollapsed= pref.isPropertyBool(UserPreference.P_HELP_COLLAPSED);
        w1.setVisible(!helpCollapsed);
                
        helpController.render(w1, this);

        if (mobile) {
        	w1.setVisible(false);
        	w1.setOpen(false);
        	Component content = w1.getFirstChild();
        	eastPopup = new Popup();
        	ToolBarButton btn = new ToolBarButton();
        	btn.setIconSclass("z-icon-remove");
        	btn.addEventListener(Events.ON_CLICK, evt -> eastPopup.close());
        ...
        contextHelp.setSclass("window-container-toolbar-btn context-help-btn");
        contextHelp.setTooltiptext(Util.cleanAmp(Msg.getElement(Env.getCtx(), "AD_CtxHelp_ID")));
        contextHelp.setVisible(!w1.isVisible());
